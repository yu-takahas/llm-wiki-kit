...
kit は `lw-` prefix で統一し、名前の形は動詞形（`lw-render` / `lw-commit`）と名詞句（`lw-retro` / `lw-doc-review`）が混在する。
...
書く対象は 3 種類で、置き場が分かれる（層分けの根拠は [[プロンプト設計原則]]「出力量の上限を規定する」セクション）。

- 応答の簡潔さ — 主要な回答に分量を割き、前置きと留保を短くする（常時ロード層）
- 作業中のナレーション — 重要な発見と方向転換のときだけ更新する（常時ロード層）
- 成果物ドキュメントの長さ — 実質に見合う分量にし、詰め物をしない（SKILL.md）

SKILL.md が持つのは 3 つ目と、その skill 自身が出す報告の分量。
応答とナレーションを skill 単位で書くと、その skill を起動していない間は効かない。

長い指示文では、末尾近くに短いリマインダーを置いて本文の指示と対にする。

出力を減らすことと、探索や走査を省略させないことは別の軸で扱う。
走査を駆動するのは手順が対象を列挙している構造であって、「該当なしでも報告する」という個別報告ではない（個別報告は自己申告で、見た証拠にならない）。
量を減らすときは個別報告を消し、skill 全体の完了報告 1 回に寄せる。

削る対象は、ステップの完了を告げるだけの報告。
次のステップが入力として使う中間出力（候補リストの列挙など）は報告でなく手順の一部なので、該当なしを含めて残す。
