...
**本ページは構成上の決定根拠のみを持つ。**
ディレクトリとファイルの実体は `templates/` 配下と kit ルートの各ファイルが正本で、本ページに写さない。
...
`templates/` 配下のファイルが `setup.sh` の cp 対象。
将来 `lw-sync` を実装する場合も同じ境界を使う（[[lw-kit-スキル設計-lw-sync]] は設計のみで未実装）。
...
空ディレクトリは `.gitkeep` で git に追跡させる。
追跡させないと、配布後にディレクトリ構造が欠けた状態で始まる。
