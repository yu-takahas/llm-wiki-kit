...
- commit の Skill 化: 当初「CLAUDE.md に数行書けば足りる → 不要」と判断 → 2026-05-25 撤回し `/lw-commit` 化した（撤回根拠は commit 単位での反映分割 / 「反映して」+「commit plz」を 1 語に畳む / dry-run 検証。詳細は [[lw-kit-スキル設計-lw-commit]]）
