...
全設計書に型を当てると、既定の「決定根拠のみ」で書けないものが出る。
4 つのケースがあり、いずれも `<持つもの>` を広げる側で扱う。宣言を諦めるのではなく、何を持つかを正確に書く。

- 正本が自分自身: 要件・ユーザー分析は実物より前に決まるので、写しの元になる実物が存在しない。`<正本のパス>` のスロットが埋まらない
- 正本が 2 系統: 手順と、判定基準を持つ別ファイルに分かれている場合。両方を名指しする
- 実物が未実装: 設計書が仕様の仮正本を兼ねる。実装後に how を移すことを宣言に書き、移行の予定まで含めて残す
- 単一の対応実物が無い: ハブや入口となる文書。正本パスを持たず「決定と根拠は各設計書が持つ」の形にする

instance が複数あっても仕様は復元できない。
同じ規約を持つ実物が 2 つあれば「共通していること」までは分かるが、どれが必須要素でどれが個別の追加かは並べても判別できない。
規範性は instance から出てこないので、要素の列挙は設計書が持ち、文言は実物が持つ。

#### 正本を動かした後に追う先

セクションを削除・改名したら、同じファイルの中でそこを指していた記述を追う。
参照だけでなく、そのセクションの語彙で書かれた判定例や、消した表を前提にした「上記」も対象になる。
他ファイルへの参照より気づきにくい。編集中のファイルは context に載っているので、読み返しても知っている内容として流れる。

正本を別ファイルに移したら、その正本を名指ししていた文書を追う。
`[[link]]` は解決するままなので lint には出ず、辿って初めて分かる。
