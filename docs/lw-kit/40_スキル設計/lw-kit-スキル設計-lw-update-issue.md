...
最小化を保つと、終端形の「☔ に `[ ]` を残さない」が割に合わない作業になる場面がある。
残った `[ ]` が多いほど Edit の往復が増えるので、一括置換に手が伸びる（実測で `sed -i` が使われた。2026-09-04、チュートリアル 01 の通し）。
それでも `sed` を足さないのは、置換対象を行頭パターンでしか縛れず、issue 本文の他の箇所まで書き換わる余地が残るため。
`git mv` を足した判断とは前提が違う。あちらは対象がファイル 1 つに閉じていて、実行結果が `git status` に出る。
...
流れの一覧と各ステップの操作手順・エラーケースは SKILL.md が持つ。ここは各ステップの why だけを置く。
