...
- 実体: SKILL.md（`templates/.claude/skills/lw-retro/SKILL.md`。導入先では `.claude/skills/lw-retro/SKILL.md` に置かれる）
...
手順の内容は SKILL.md が正本。本節は順序の根拠だけを持つ。

| #   | 責務                 | 順序の根拠                                                                          |
| --- | -------------------- | ----------------------------------------------------------------------------------- |
| 1   | セッション読み返し   | 探索の材料を集める。context が greedy に縮んだ時の外部記憶として jsonl は明示時のみ |
| 2   | 観察収集             | 本 skill の肝。読み返した材料から探索的に観察を引き出す                             |
| 3   | リファクタ検討       | 棚卸し＝探索の所作。変更量ベースの優先順位で無駄な走査を防ぐ                        |
| 4   | 反映先判断           | 再発は事例行 +1、新規は骨子提示                                                     |
| 5   | 骨子提示・確認・反映 | 反映は副作用があるので確認を挟む。探索の出口                                        |
...
参照先を開く動作と走査の範囲を、参照の所在とは別に手順側へ書く。
参照の所在だけを示した版を実タスクで測ったところ、15 ファイルを変更した回で `wiki-style.md` が一度も開かれず、走査も変更量が最大の 1 ファイルで終わった。
残りは記憶で「追記のみで劣化なし」と判定されている。
同じ回で責務 4 の重複検出は動いており、差は文の形にあった。
責務 4 は grep という動作を渡し、責務 3 は参照先の名前を渡していた。
言い訳対戦表の「触ったのは一部だが全部見るのは面倒」は当時も存在したので、表だけでは走査の範囲が保たれない。

`wiki-style.md` の `paths` はワークスペースのコンテンツディレクトリに向いており、`.claude/` や `00_issues/` だけを触った回では条件ロードが発火しない。
retro の変更ファイルはこれらに寄ることがあるため、手順側がパスで指す。

責務 3 が扱うのは page の構成が主題と合っているかで、機械処理の後始末は含めない。
後者は読めば終わる確認で、セッション開始時に読まれる行動指針が既に持っている。
同じ責務に両方を置くと、後始末を済ませた時点で構成の見直しまで終わったことになる。
...
件数でなく「領域」を強制する。
反映先の各領域を一巡問う（領域の列挙と問いかけの文言は SKILL.md）。
...
`/lw-tdd` の Red-Green-Refactor 規律が崩れていないかを点検する（点検項目と起動条件は SKILL.md）。
...
具体プロンプト文言（領域走査の問いかけ / モード宣言の文）は SKILL.md が持つ。
設計書は構造と why まで、文言は SKILL.md に置く（設計書 = why / SKILL.md = how の分担、[[Progressive-Disclosure]] 思想）。

## 探索の出口: 再発の扱い

既存項目と被る観察は drop せず、再発として扱う（記録の形は台帳の追記ルールが正本）。
...
重複検出は記憶でなく検索で行う。
記憶だけで判定すると、既に書かれている観察を新規として再提案する。
根拠は [[プロンプト設計原則]]「実物を開いてから書く」で、調査結果を書く前に一次ソースを開くのと同じ適用になる。
範囲を反映先に閉じるのは、責務 4 の起動コストを上げないため。
...
実装ファイル: SKILL.md（`templates/.claude/skills/lw-retro/SKILL.md`）
