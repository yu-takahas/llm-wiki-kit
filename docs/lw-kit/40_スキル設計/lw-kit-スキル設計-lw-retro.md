...
description は [[Claude-Code-Skillの書き方]] の英語・third person 規約に反して日本語のまま維持する。
`disable-model-invocation: true` で description は model の自動起動判定 context に載らない（手動 `/lw-retro` 起動のみ）ため、英語化の実利が無い（[[lw-kit-スキル設計-lw-commit]] と同じ判断）。
将来 model 自動起動を有効化する場合は英語・三単現に直す。

## 許可ツールの最小化

Read / Edit / Write / Glob + scoped Bash(`git diff` / `git status` / 棚卸しチェックスクリプト)。
汎用 Bash は持たない。
Glob は doc-review 宣言（`.doc-review.md`）の発見に使う。
dot ファイルなので `50_feedback/` の `ls` 走査に出ず、`Bash(find)` を足すより doc-review 側（`Glob(**/.doc-review.md)`）と手段を揃える方が一貫する。
Write を含むのは反映で新規 page / memory を作る場合があるため（要確認側、「反映の自走度」参照）。
git は read-only（diff / status のみ、add / commit は `/lw-commit` の責務）。
...
| 4   | 反映先判断           | 反映先を判断（作業スタイル / `50_feedback/` / `30_wiki/` / `40_project/` / memory）                                                 | 再発は事例行 +1、新規は骨子提示                                                     |
...
[[lw-kit-スキル設計-lw-fix-review]] の採否 4 区分はこの用途にはリッチすぎるので、軽微 / 要確認の 2 段に簡略化する。
...
- `/lw-retro` 自身は反映先（作業スタイル / page / memory）の更新以外のファイルを触らない（振り返りフローに閉じる、git は read-only）

## 未解決 / SKILL.md への持ち越し

確定事項は本文各セクションに溶かし込んだ。
残る未確定と、SKILL.md 実装時に詰める事項をここに集約する（各所の inline 言及はそのまま残す）。

- doc-review 宣言点検の起動ゲート: レビューを回した / 設計書を worker に渡したセッションに限る方針は確定、判定条件は SKILL.md 実装時（「探索の入口」参照）
- 領域走査の具体的問いかけ / モード宣言文（D の文言）: 構造と why は確定、文言は SKILL.md 実装時（「探索の入口」参照）
- 重要度で間引く具体基準: UCB 的に変化の大きい領域を優先する等の基準は SKILL.md 実装時に詰める（歯止めの二段構えそのものは「探索の入口」「探索の出口」で確定済み）
- jsonl 読みのコスト: 明示指示時のみ読む方針は確定。読み方は「全文を context に載せず `role` 別に抽出して粒度を絞る」（user 発話と assistant のテキスト応答だけを切り出し、tool_result / system reminder は捨てる）。全文ロードは context を greedy に縮めて探索を阻害するため、外部記憶としての jsonl は抽出経由で当たる。具体の抽出範囲（直近 N ターン / 全域）は SKILL.md 実装時（責務 1 参照）
...
- [[lw-kit-アーキテクチャ設計]] — skill 群全体での位置づけ（背骨チェーンの「振り返り」段）
