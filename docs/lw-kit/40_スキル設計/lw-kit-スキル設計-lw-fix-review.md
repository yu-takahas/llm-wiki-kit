...
書き込み面を allowlist で制限する理由: 自走範囲を広げた（相談トリガーを閉じた条件に限定した）代わりに、書き込み面の予測可能性で安全性を担保する。allowlist + `git diff` 事後確認の 2 つが自走拡大の前提条件。
...
レビューフロー全体は [[Self-Refine]] の人間ループ版を Claude 内で自動化する位置づけ。

## 入力

複数形式を受け付ける理由: reviewer 側の出力先統一が完了するまでの後方互換。
...
lead 相談を独立区分にすることで、lead 単独判断のラインを明示する（過去に「字面ルールの機械適用」で lw-cmux-teams 側に起きた事故と同種のものを防ぐ）。
...
- 完了条件を設ける。理由: 意識に頼る蓄積は回らない（[[レビューワークフローの設計原則]] の原則 5）。具体的な条件は SKILL.md 参照
...
全 review skill の出力先は `/tmp/lw-review/<issue-name>/YYYYMMDD-HHMM-<名前>.md` に統一している。
...
昇格 3 条件: 複数回再現 + 毎回採用 + 機械的に照合可能（「〜してはならない / 〜すること」の規則文に書き直せる）。
...
- 確認（事実確認）は常に lead に回ること — 外れると幻覚修正が起きる
...
- [[lw-kit-アーキテクチャ設計]] — skill 群全体での位置づけ（背骨チェーンの「レビュー」段）
