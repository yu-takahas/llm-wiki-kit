...
**帰属は基準の出所で決まる**: 同じ指摘でも、通例が出したものが seed に昇格すると、次回からは骨格の担当になる。
境界事例表の判定理由に基準の出所を必ず書くのはこのため。

**起動条件が他と違う理由**: `--full` でのみ起動する。
web search の待ち時間とトークンコストが乗るので、デフォルトに常時載せると、レビューを回すこと自体を避ける方向に働く。
上澄みを脱する価値は「腰を据えて見るとき」に出るものと見て、明示オプトインに寄せた。
...
アンカー無効化時の扱い（骨格修正で下位層の引用が壊れた場合）は、注記でなく引用の生死で判定する。
...
「あり / なし」を常に書く形にしたのは、条件に当てはまらない回に何も書かないと、規則が空振りしたのか判定を飛ばしたのかが区別できないため。
